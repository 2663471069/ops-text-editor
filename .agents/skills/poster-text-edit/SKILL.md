...
7. When replacement data includes `selectedFont`, use the exact trusted local font file and variant listed there. Do not substitute or approximate another typeface. Preserve the original target text's color and effects while using the selected font's glyph shapes.
8. Preserve every unlisted word and all layout, dimensions, colors, people, products, logos, background, lighting, and visual style. Pixels outside the target text area should remain visually unchanged.
9. Do not add watermarks, explanations, decorative text, or redesign elements.
10. Verify that every replacement appears exactly, matches the source text styling, and that unrelated content remains unchanged. Make at most one targeted retry when a replacement is missing, misspelled, or visibly restyled.
11. Save the final raster image exactly to the requested output path. Convert formats only when required by the requested extension.
