...
export function buildCodexPrompt({ changes, outputPath, templatePrompt = '' }) {
  ...
  const sections = [
    ...
    'For every replace_text operation, first inspect the original text inside the specified box and use it as the visual style reference. Replace only the glyph content; do not redesign or restyle the text.',
